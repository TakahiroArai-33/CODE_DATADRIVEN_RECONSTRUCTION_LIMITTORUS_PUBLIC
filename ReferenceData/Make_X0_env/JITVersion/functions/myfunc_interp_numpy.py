...
def myfunc_interp_numpy(x_new, xp, Yp):
    ...
    (xp, Yp):
        xp: 1d
        Yp: 2d [xp.size, rx.size]
    Perform linear interpolation over (xp, Yp).

    Returns:
        Yp(x_new)
    """
    if xp.size != Yp.shape[0]:
        raise ValueError("xp and Yp have incompatible sizes")
    if len(Yp.shape) != 2:
        raise ValueError("Yp must be a two-dimensional array")
    if len(xp.shape) != 1:
        raise ValueError("xp must be a one-dimensional array")
    # Reject non-scalar interpolation points.
    ...
    # Clamp values outside the xp range to the nearest endpoint.
    if idx == 0:
        return Yp[0]
    elif idx == len(xp):
        return Yp[-1]
