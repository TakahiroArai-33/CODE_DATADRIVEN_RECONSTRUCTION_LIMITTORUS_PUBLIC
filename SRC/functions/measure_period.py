...
def return_to_section(X, rx, t, section, key:Literal["U", "V"], index=0,
                      display=True):
    """
    Return the state and time at an upward Poincare-section crossing.

    The section is defined using the spatial-phase-independent mode H0 of U or
    V. ``index`` selects the zero-based crossing counted from the start.
    """
    ...
    if display:
        ...
        print("Using upward crossing number {:d}.".format(index+1))

    if args.sum() == 0:
        print("No upward crossing of the section was found.")
        return None

    # Indices of crossings in the Boolean mask.
...
def measure_period(X, rx, t, section, key="U", index=0):
    """Measure the period between two successive section crossings."""
